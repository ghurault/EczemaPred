...
The priors are passed as a named list with elements `sigma`, `y_inf` and
`slope` specifying priors for the corresponding parameters. Each element
of the list should be a vector of length 2, containing values for x1 and
x2, x2 \> 0, such as:
...
the parameter is constrained to be positive. NB: For `slope`, both `x1`
and `x2` must be positive.

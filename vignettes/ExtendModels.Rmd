---
title: "Extending EczemaPred models"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Extending EczemaPred models}
...
The package uses the S3 object oriented programming system in R to allow inheritance and polymorphism.
For more details of the S3 OOP system, you can check [Hadley Wickham's book](https://adv-r.hadley.nz/s3.html).

The package provides a set of generics that can be useful when extending a models:

- `default_prior()`: Provide default priors
- `validate_prior()`: Check that the prior is correctly specified
- `prepare_standata()`: Prepare data input to the Stan sampler
- `list_parameters()`: List parameters of the model
- `print_prior()`: How to display priors in the `print` method (optional)
- `EczemaFit()`: Fit model (default generic is only valid with models provided in the package)
- `sample_prior()`: Sample prior predictive distribution of the model (default generic is only valid with models provided in the package)
...
In the original Stan model, available [here](https://github.com/ghurault/EczemaPred/blob/main/inst/stan/BinRW.stan), this would mean:
...
- Include the prior for `beta_sex` in the model block, for example: `sex ~ normal(0, 1)`.
The prior can also be specified as data in the data block with `real prior_beta_sex[2];` and in the model block `sex ~ normal(prior_beta_sex[1], prior_beta_sex[2]);`

Let's call this new model `BinRW2`.

## Updating the pre-processing

One way to use the package infrastructure to minimise the amount of new code to write would be to create a new model class `BinRW2` that inherits from the class `BinRW`.
We also write `BinRW2` methods for `default_prior`, `validate_prior` and `print_prior` which makes use of `NextMethod()` to call the metods for the parent class `BinRW`.
...
To sample the prior predictive disribution, we need to generate an empty dataset to pass to `prepare_standata` and then call `rstan::stan` (the code could be put into a function `sample_prior.BinRW2`).
